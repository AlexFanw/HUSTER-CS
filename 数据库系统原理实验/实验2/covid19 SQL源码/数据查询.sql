...
SELECT DISTINCT station.CityName FROM station,TakeTrainRecord
	WHERE station.SID=TakeTrainRecord.AStationID -- 出发的地方是SID
	AND TakeTrainRecord.SStationID IN(
...
SELECT train.* FROM train WHERE train.SDate='2020-01-22' 
	AND train.SStationID -- 出发的车站SID为武汉的SID
...
WHERE passenger.PCardID = TakeTrainRecord.PCardID -- 连接身份证号
AND station.SID= TakeTrainRecord.AStationID -- 连接到达城市
AND train.TID = TakeTrainRecord.TID -- 连接车次号
AND TakeTrainRecord.TID IN(
  SELECT DISTINCT train.TID FROM train WHERE train.SDate='2020-01-22'  -- 时间是2020-01-22
AND train.SStationID IN ( 
		SELECT DISTINCT SID FROM station WHERE station.CityName= '武汉')); -- 从武汉离开
		

-- 7）统计“2020-01-22” 从武汉离开的所有乘客所到达的城市及达到各个城市的武汉人员数。
SELECT station.CityName,count(passenger.PCardID) FROM passenger,TakeTrainRecord,station,train
WHERE passenger.PCardID = TakeTrainRecord.PCardID  -- 连接身份证号
AND station.SID= TakeTrainRecord.AStationID -- 连接站台号
AND train.TID = TakeTrainRecord.TID -- 连接车次号
AND TakeTrainRecord.TID IN(
	SELECT DISTINCT train.TID FROM train WHERE train.SDate='2020-01-22'  -- 时间限定
	AND train.SStationID IN (SELECT DISTINCT SID FROM station WHERE station.CityName= '武汉')) -- 地点限定
GROUP BY(CityName);


-- 8）查询2020年1月到达武汉的所有人员；
SELECT DISTINCT passenger.* , train.ATime FROM passenger,TakeTrainRecord,station,train
	...
	AND train.ATime >= '2020-01-01' AND train.ATime < '2020-02-01';
...
SELECT count(DISTINCT PCardID) FROM TakeTrainRecord
WHERE (TakeTrainRecord.TID,TakeTrainRecord.CarrigeID) IN(
	SELECT TakeTrainRecord.TID,TakeTrainRecord.CarrigeID -- 车次号，车厢号
	FROM passenger,TakeTrainRecord,train,DiagnoseRecord
	WHERE TakeTrainRecord.TID = train.TID
	  -- AND train.ATime =  '2020-01-22' -- 时间
	  -- AND train.TName = 'G007'  -- 车次
...
WHERE train.TID = TakeTrainRecord.TID
	...
	AND DiagnoseRecord.DStatus = 1
	GROUP BY train.TID,TakeTrainRecord.CarrigeID
	HAVING count(*)<=3; -- 数据量太小了,所以改成了小于等于3
...
SELECT PCardID,train.TID,SDate FROM TakeTrainRecord,train
WHERE TakeTrainRecord.TID = train.TID
AND SDate = '2020-01-22' 
AND train.SStationID = (SELECT SID FROM station WHERE SName ='武汉')
-- AND STime < ALL(
-- 		SELECT STime  FROM TakeTrainRecord,train
-- 		WHERE SDate = '2020-01-22'
-- 		AND TakeTrainRecord.TID = train.TID
-- 		AND train.SStationID IN (SELECT SID FROM station WHERE SName <> '武汉')) -- 首发必须是武汉站
AND PCardID IN (
		SELECT PCardID FROM TakeTrainRecord,train
		WHERE SDate = '2020-01-22'
		AND TakeTrainRecord.TID = train.TID
		AND train.SStationID IN (SELECT SID FROM station WHERE SName <> '武汉'))
...
-- 16）查询所有发病日期相同而且确诊日期相同的病患统计信息
-- 包括：发病日期、确诊日期和患者人数，结果按照发病日期降序排列的前提下再按照确诊日期降序排列。
